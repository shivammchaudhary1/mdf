...
export class SecurityMiddleware implements NestMiddleware {
  ...
  private localLimit(request: Request, response: Response) {
    ...
    if (this.requests % 1000 === 0) {
      for (const [k, v] of this.localBuckets) if (v.resetAt <= now) this.localBuckets.delete(k);
  ...
  private enforceOrigin(request: Request) {
    ...
    if (origin && !this.origins().includes(origin)) throw new ForbiddenException("Request origin is not allowed.");
    if (request.headers["sec-fetch-site"] === "cross-site") throw new ForbiddenException("Cross-site request is not allowed.");
  }
  private enforceCsrf(request: Request) {
    if (["GET", "HEAD", "OPTIONS"].includes(request.method)) return;
    const route = this.route(request);
    const exempt = ["/auth/login", "/auth/google", "/auth/register", "/auth/forgot-password", "/auth/reset-password", "/contact"].some(
      (v) => route.endsWith(v),
    );
    if (exempt) return;
  ...
  async use(request: Request, response: Response, next: NextFunction) {
    ...
    if (this.route(request).includes("/auth/")) response.setHeader("Cache-Control", "no-store");
    next();
  }
}
