...
export class AuthController {
  ...
  private issueCsrf(response: Response) {
  ...
  private clearCookies(response: Response) {
    ...
    response.clearCookie("mdadu_csrf", this.csrfCookieOptions());
  }

  @Post("register") async register(@Req() request: Request, @Body() input: RegisterDto, @Res({ passthrough: true }) response: Response) {
    return this.setSession(response, await this.auth.register(input, this.requestContext(request)));
  }
  @Post("login") async login(@Req() request: Request, @Body() input: LoginDto, @Res({ passthrough: true }) response: Response) {
    return this.setSession(response, await this.auth.login(input, this.requestContext(request)));
  }
  @Post("google") async google(@Req() request: Request, @Body() input: GoogleAuthDto, @Res({ passthrough: true }) response: Response) {
    return this.setSession(response, await this.auth.googleLogin(input, this.requestContext(request)));
  }
  @Post("logout") async logout(@Req() request: Request, @Res({ passthrough: true }) response: Response) {
    this.clearCookies(response);
    return this.auth.logout(sessionToken(request));
  }
  @Post("logout-all") @ApiCookieAuth() @UseGuards(SessionGuard) async logoutAll(
  ...
  @Patch("account") @ApiCookieAuth() @UseGuards(SessionGuard) updateAccount(
    @Req() request: AuthRequest,
