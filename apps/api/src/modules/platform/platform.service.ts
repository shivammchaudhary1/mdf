...
import { ContentKind, ContentRecord, contentKinds } from "./platform.models";
...
export class PlatformService {
  ...
  private kind(k:string){if(!(contentKinds as readonly string[]).includes(k))throw new NotFoundException("Content type not found.");return k as ContentKind}
  ...
  private serialize(item:ContentRecord|Record<string,unknown>){
    ...
    return {...item,_id:String((item as{_id:unknown})._id),projectId:record.projectId?String(record.projectId):undefined,coverMediaId:cover,coverImage:cover?this.media.urlsFor(cover).large:undefined,mediaIds:ids,media:ids.map(id=>this.media.urlsFor(id).large)};
  }
  ...
  async publicItem(kind:string,slug:string){
    const item=await this.content.findOne({kind:this.kind(kind),slug,published:true,archived:false,$or:[{publishedAt:{$exists:false}},{publishedAt:null},{publishedAt:{$lte:new Date()}}]}).lean();
  ...
  private async validate(input:ContentDto|UpdateContentDto,actorId:string){
    if(input.data&&(Object.keys(input.data).length>60||Object.entries(input.data).some(([k,v])=>!/^[a-zA-Z][a-zA-Z0-9_]{0,60}$/.test(k)||typeof v!=="string"||v.length>10000)))throw new BadRequestException("Settings values must be short named text fields.");
    await this.media.assertOwnedBy(actorId,[input.coverMediaId,...(input.mediaIds??[])]);
    if(input.projectId&&!(await this.projects.exists({_id:objectId(input.projectId,"Project not found."),archived:false})))throw new BadRequestException("Project not found.");
  }
  ...
  async create(kind:string,input:ContentDto,actorId:string){
    ...
    }catch(error:unknown){if(error&&typeof error==="object"&&"code"in error&&error.code===11000)throw new ConflictException("This content slug is already in use.");throw error}
  }
  ...
  async update(kind:string,id:string,input:UpdateContentDto,actorId:string){
    ...
    const item=await this.content.findOneAndUpdate({_id:objectId(id),kind:k,archived:false},{$set:update},{new:true,runValidators:true});
    if(!item)throw new NotFoundException("Content not found.");
    if(item.published)await this.media.makePublic([item.coverMediaId?String(item.coverMediaId):undefined,...(item.mediaIds??[]).map(String)]);
  ...
  async archive(kind:string,id:string,actorId:string){
    ...
    const item=await this.content.findOneAndUpdate({_id:objectId(id),kind:k},{$set:{archived:true,published:false,updatedBy:new Types.ObjectId(actorId)}},{new:true});
    if(!item)throw new NotFoundException("Content not found.");
