...
export class CastingService {
  ...
  private serialize(
    casting: Casting | Record<string, unknown>,
  ) {
  ...
  private async validate(
    input: CreateCastingDto | UpdateCastingDto,
  ) {
    if (
    ...
    if (
      input.deadline &&
      input.shootDate &&
      new Date(input.deadline) > new Date(input.shootDate)
    ) {
      throw new BadRequestException(
        "Application deadline cannot be after the shoot date.",
      );
    }
    ...
    try {
      ...
      if (
        ...
        throw new ConflictException(
          "This casting-call slug is already in use.",
        );
      }
  ...
  async update(
    ...
    if (input.coverMediaId) {
      update.coverMediaId = new Types.ObjectId(
        input.coverMediaId,
      );
    }
