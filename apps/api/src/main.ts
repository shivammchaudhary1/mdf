...
async function bootstrap() {
  ...
  const expressApp = app.getHttpAdapter().getInstance() as { disable(name: string): void; set(name: string, value: unknown): void };
  expressApp.disable("x-powered-by");
  if (config.get<boolean>("TRUST_PROXY")) expressApp.set("trust proxy", 1);
  ...
  const frontendUrls = String(config.get("FRONTEND_URL") ?? "http://localhost:3333")
    .split(",")
    .map((v) => v.trim())
    .filter(Boolean);
  app.enableCors({
  ...
  if (config.get<boolean>("SWAGGER_ENABLED")) {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription("REST API for the M. Dadu Films Digital Platform V1")
      .setVersion("1.0.0")
  ...
  if (config.get<boolean>("MONGODB_AUTO_INDEX")) {
    const connection = app.get<Connection>(getConnectionToken());
    await Promise.all(Object.values(connection.models).map((model) => model.init()));
  }
  const port = Number(config.get("PORT") ?? 8888);
  await app.listen(port, "0.0.0.0");
  console.log(`M. Dadu Films API: http://localhost:${port}/api/v1`);
  if (config.get<boolean>("SWAGGER_ENABLED")) console.log(`Swagger: http://localhost:${port}/docs`);
}
void bootstrap();
