...
export type ApiContent=ContentItem&{_id:string;published:boolean;archived:boolean;data?:Record<string,string>;images?:string[];videoUrl?:string;requirements?:string;compensation?:string;credits?:string;acceptingApplications?:boolean};

async function publicFetch<T>(path:string):Promise<T>{const response=await fetch(`${runtimeConfig.apiUrl}${path}`,{cache:"no-store"});if(!response.ok)throw new Error("Content is temporarily unavailable.");return response.json()}

export async function getCollection(kind:CollectionKind,page=1,limit=20){
  const result=await publicFetch<{items:(ApiContent&{coverImage?:string})[];meta:PageMeta}>(`/content/${kind}?page=${page}&limit=${limit}`);
...
export async function getContentItem(kind:CollectionKind,slug:string){
  ...
  if(response.status===404)return undefined;if(!response.ok)throw new Error("Content is temporarily unavailable.");
...
export async function getSettings(slug:string){
  ...
  if(response.status===404)return{};if(!response.ok)throw new Error("Settings are temporarily unavailable.");
  const record=await response.json() as ApiContent;return record.data??{};
}
