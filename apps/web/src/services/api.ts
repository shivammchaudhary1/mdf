...
async function loadCsrf() {
  ...
  csrfPromise = fetch(`${runtimeConfig.apiUrl}/auth/csrf`, { credentials: "include", cache: "no-store" })
    .then(async (r) => {
      if (!r.ok) return null;
      const d = await r.json().catch(() => null);
      csrfToken = typeof d?.csrfToken === "string" ? d.csrfToken : null;
      return csrfToken;
    })
    .finally(() => {
      csrfPromise = null;
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (response.status === 401 || (path === "/auth/logout" && response.ok)) clearApiSecurityState();
  if (!response.ok) {
