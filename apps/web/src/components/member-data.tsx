...
export function MemberData({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    const [account, dashboard, applications, opportunities, posts] = await Promise.all([
      api<MemberProfile>("/member/profile"), api<Dashboard>("/member/dashboard"),
      allPages<ApplicationRecord>("/member/applications"), allPages<OpportunityRecord>("/member/opportunities"), allPages<ContentRecord>("/content/blog"),
    ]);
    const p = account.profile ?? {}; const a = account.account; const counts = dashboard.applicationSummary;
    setValue({ memberId: a.id, profile: p, data: {
      member: { name:a.name,firstName:a.name.split(" ")[0],email:a.email,mobile:a.mobile,location:p.city??"",profession:p.profession??"",verified:a.verified,profileCompletion:account.completion,availability:p.availability??"",memberSince:p.createdAt?new Date(p.createdAt).getFullYear().toString():"—",photo:mediaUrl(p.photo) },
      stats:[{label:"Applications",value:String(counts.total),helper:`${counts.submitted + counts.underReview} active`},{label:"Shortlisted",value:String(counts.shortlisted),helper:""},{label:"Profile views",value:"—",helper:""},{label:"Saved roles",value:String(p.savedOpportunityIds?.length??0),helper:""}],
  ...
  useEffect(()=>{void refresh().catch(error=>{if(error instanceof ApiError&&error.status===401)router.replace("/login");else toast.error(error instanceof Error?error.message:"Unable to load member data.")});},[refresh,router,toast]);
