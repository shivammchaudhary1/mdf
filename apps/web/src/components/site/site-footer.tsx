    ...
    <footer className="border-t border-black/8 bg-white">
      <div className="site-shell grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-[1.25fr_.75fr_.75fr_1fr] lg:py-14">
        ...
        <div>
          ...
          {entries.length ? (
            <div className="mt-4 flex gap-2">
              {entries.map(([name, href]) => (
                <a
                  ...
                  rel="noreferrer"
                  aria-label={name}
                  className="grid h-9 w-9 place-items-center rounded-full border border-black/10 text-[#333] transition hover:bg-black hover:text-white"
                >
                  <Icon name={name} />
                </a>
              ))}
            ...
            <p className="mt-4 max-w-[220px] text-xs leading-5 text-[#888]">
              Official social links will appear here when added in Company Settings.
            </p>
          )}
        </div>
        <div>
          <p className="site-footer-title">Work with us</p>
          <p className="mt-4 text-sm leading-6 text-[#777]">Explore production, creative and platform opportunities with M. Dadu Films.</p>
          <Link href="/careers" className="site-text-link mt-4 inline-flex">
            View Careers →
          </Link>
        </div>
      </div>
      <div className="site-shell flex flex-wrap items-center justify-between gap-3 border-t border-black/5 py-5 text-[11px] text-[#888]">
        ...
        <div className="flex flex-wrap gap-5">
          ...
          <a href="/sitemap.xml" className="hover:text-black">
            Sitemap
          </a>
        </div>
      </div>
