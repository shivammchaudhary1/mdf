...
export function AdminShell({section,children}:{section:string;children:ReactNode}){
 ...
 const data={admin:{name:user?.name??"",initials:(user?.name??"").split(" ").map(x=>x[0]).join("").slice(0,2)}};
 useEffect(()=>{void api<CurrentUser>("/auth/me").then(user=>{if(user.role!=="SUPER_ADMIN")router.replace("/member");else setUser(user);}).catch(()=>router.replace("/login"));},[router]);
 async function logout(){try{await api("/auth/logout",{method:"POST"});router.replace("/login");}catch(error){toast.error(error instanceof Error?error.message:"Unable to sign out.");}}
 ...
 return <div className="ad-shell">
  ...
  <div className="ad-main">
   ...
   <main className="ad-content">{user?children:null}</main>
  </div>
 </div>
}
