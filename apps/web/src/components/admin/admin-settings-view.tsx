...
export function AdminSettingsView({legal=false}:{legal?:boolean}){
 ...
 async function savePolicy(event:FormEvent<HTMLFormElement>){
  event.preventDefault();if(!policy||saving)return;const form=new FormData(event.currentTarget);setSaving(true);
  try{await persist(policy,policy==="privacy"?"Privacy Policy":"Terms of Use",{body:[String(form.get("content")??"")],published:form.get("published")==="on"});toast.success("Policy saved.");setPolicy(null);}
  catch(error){toast.error(error instanceof Error?error.message:"Unable to save policy.");}finally{setSaving(false);}
 }
 ...
 if(legal)return <div className="ad-stack">
  ...
  <section className="ad-settings-grid">
   <div className="ad-form-stack" ref={fields} key={JSON.stringify(legal?data.legal:data.settings)}>
    ...
    <button className="ad-primary ad-save" disabled={saving} onClick={save}>{saving?"Saving…":"Save Legal Content"}</button>
   </div>
  ...
  <AdminDialog open={!!policy} onClose={()=>setPolicy(null)} eyebrow="Legal content" title={policy==="privacy"?"Privacy Policy":"Terms of Use"} description="Draft the policy UI here. Have final legal wording reviewed before publication." width="wide">
    <AdminDialogForm onSubmit={savePolicy}>
      <AdminFormField label="Policy Content"><textarea name="content" defaultValue={records.find(x=>x.slug===policy)?.body?.join("\n\n")??""} rows={14} placeholder="Enter reviewed legal policy text here."/></AdminFormField>
      <label className="ad-dialog-check"><input type="checkbox" name="published" defaultChecked={records.find(x=>x.slug===policy)?.published??false}/><span>Mark as ready to publish after legal review</span></label>
