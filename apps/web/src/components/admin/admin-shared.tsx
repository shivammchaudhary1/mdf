...
export function AdminPrimaryButton({ children, onClick }: { children: ReactNode; onClick?: () => void }) {
  return (
    <button type="button" className="ad-primary" onClick={onClick}>
      <AdminIcon name="plus" />
      {children}
    </button>
  );
...
export function AdminFilters({ values, active, onChange }: { values: string[]; active: string; onChange: (v: string) => void }) {
  return (
    <div className="ad-filters">
      {values.map((v) => (
        <button type="button" key={v} onClick={() => onChange(v)} className={active === v ? "active" : ""}>
          {v}
        </button>
      ))}
    </div>
...
export function AdminSearch({
  ...
  return (
    <label className="ad-section-search">
  ...
  if (error)
    return (
      <div className="ad-empty">
        <span>{error}</span>
        {onRetry && (
          <button type="button" className="ad-dialog-secondary" onClick={onRetry}>
            Try Again
          </button>
        )}
      </div>
  ...
  return (
    <span className={`ad-more-wrap ${open ? "is-open" : ""}`}>
        ...
        <>
          ...
          <span className="ad-more-menu">
            ...
            <button
              ...
              onClick={() => {
                setOpen(false);
                if (onArchive) setConfirm(true);
                else toast.info("Archiving is not available for this item.");
              }}
            >
              Archive
            </button>
      ...
      <ConfirmDialog
        ...
        description="The item will no longer be published."
