...
export default function CareersPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset();
      toast.success("Application received.", "Thank you. The team will contact you if your profile matches an opening.");
    } catch (error) {
      toast.error("Unable to submit application.", error instanceof Error ? error.message : undefined);
    } finally {
      setSaving(false);
    }
  }
    ...
    <>
      ...
      <main id="main-content">
        <PageIntro
          ...
          description="Share your profile for production, creative, operational or platform opportunities."
          mediaAlt="Film production team"
          mediaKind="team"
        />
        <section className="site-section bg-[#fafafa]">
          <div className="site-shell grid gap-8 lg:grid-cols-[.75fr_1.25fr]">
            <aside className="site-card h-fit p-7 sm:p-8">
              ...
              <h2 className="font-display mt-3 text-3xl font-semibold">Show us how you like to work.</h2>
              <p className="mt-5 text-sm leading-6 text-[#666]">
                Use shareable HTTPS resume and portfolio links. Applications are reviewed against current and upcoming openings.
              </p>
            </aside>
            <form onSubmit={submit} className="site-card grid gap-5 p-7 sm:p-9">
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <label className="grid gap-2 text-sm font-semibold">
                  Resume URL
                  <input className="field" name="resumeUrl" type="url" placeholder="https://..." />
                </label>
                <label className="grid gap-2 text-sm font-semibold">
                  Portfolio URL
                  <input className="field" name="portfolioUrl" type="url" placeholder="https://..." />
                </label>
                <label className="grid gap-2 text-sm font-semibold sm:col-span-2">
                  LinkedIn URL
                  <input className="field" name="linkedinUrl" type="url" placeholder="https://..." />
                </label>
                <label className="grid gap-2 text-sm font-semibold sm:col-span-2">
                  Tell us about yourself
                  <textarea className="field min-h-36" name="coverNote" required minLength={20} maxLength={5000} />
                </label>
              </div>
